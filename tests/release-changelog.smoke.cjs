...
const expectedVersion = "2026.09.17.1";
...
(async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /Формы превращающихся моделей собраны вместе/iu,
    );
    assert.match(await banner.innerText(), /v2026\.09\.17\.1/iu);
    ...
    assert.equal(await changelog.locator(".changelog-entry").count(), 7);
