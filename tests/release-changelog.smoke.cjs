...
const expectedVersion = "2026.09.20.1";
...
(async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /Sniveling Coward можно заменить в арсенале/iu,
    );
    assert.match(await banner.innerText(), /v2026\.09\.20\.1/iu);
    ...
    assert.equal(await changelog.locator(".changelog-entry").count(), 9);
