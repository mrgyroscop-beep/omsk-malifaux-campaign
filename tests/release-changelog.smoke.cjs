...
const expectedVersion = "2026.09.23.1";
...
(async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /Эффекты Lucky Miss добавляются прямо к модели/iu,
    );
    assert.match(await banner.innerText(), /v2026\.09\.23\.1/iu);
    ...
    assert.equal(await changelog.locator(".changelog-entry").count(), 10);
